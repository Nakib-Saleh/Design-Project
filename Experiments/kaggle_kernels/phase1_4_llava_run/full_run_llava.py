"""CULPRIT-VQA Phase 1-4 -- SECOND MODEL comparison run (LLaVA-OneVision-0.5B).

Identical experiment design to the main Qwen2.5-VL-3B run (same 150
CVQA items selection procedure, same operators, same k=2/k=3 mix, same
seed) but with a different, smaller model family -- so the per-factor
blame pattern found on Qwen can be checked against a genuinely different
model rather than resting on one model's quirk.

Writes /kaggle/working/phase1_4_llava_results.jsonl -- one JSON record
per item, appended after every item so a quota cutoff loses at most one.
"""
...
RESULTS_PATH = "/kaggle/working/phase1_4_llava_results.jsonl"
N_ITEMS = 150
...
def build_item(ex, index: int) -> Item:
    ...
    return Item(
        item_id=f"cvqa_llava_{index:04d}",
...
def main():
    ...
    t0 = time.time()
    runner = HFVisionLanguageRunner(model_id="llava-hf/llava-onevision-qwen2-0.5b-ov-hf", seed=SEED)
