...
RESULTS_PATH = "/kaggle/working/natural_failure_results_7b.jsonl"
N_ITEMS = 220  # pool of clean items to test; only naturally-wrong ones get a full record

# Second model, so a diagnosis can be checked for being a property of the
# ITEMS rather than of one model's quirks. LLaVA-OneVision-7B rather than
# the 0.5B used in the phase 1-4 comparison: a 0.5B model's mistakes are
# overwhelmingly "too weak to do the task", which no repair can fix, so
# its audit would return ~everything unexplained and say nothing about
# whether the diagnosis method works. Feasibility-probed on a T4 before
# launching: 10.3/15.6 GB peak, 10.4s median call, 10/10 calls clean.
MODEL_ID = "llava-hf/llava-onevision-qwen2-7b-ov-hf"

# Same pool, same size, same ordering as the Qwen-3B audit, so the two
# runs are comparable item-for-item rather than only in aggregate.
